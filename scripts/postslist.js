...
function addPosts() {
    posts.forEach(function(value, index) {
        ...
        var post = '<div class="post-item" id=' + postId + '>' + 
        '<div class="post-author">' + value.postAuthor + '</div>' +
        '<div class="post-body">' + 
        '<div class="post-title">' + value.postTitle +
        '<i class="fa fa-trash trash-icon" onclick="showDeletePostForm('+ postId +')"></i>' +
        '</div>' +
        '<div class="post-content">' + value.postContent +
        '</div>' +
...
function deletePost() {
    ...
    var id = document.getElementById('no-post-btn').getAttribute('delete-post-id').slice(7);
    console.log(id);
